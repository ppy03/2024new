...
const store = {
	...
	state: {
		...
		config: {
			quality: String(QUALITY_HIGH), // will be mirrored to a global variable named `quality` in `configDidUpdate`, for perf.
			shell: 'Random',
			size: IS_DESKTOP
				? '4' // Desktop default
				: IS_HEADER 
...
function configDidUpdate() {
	...
	Spark.drawWidth = quality === QUALITY_HIGH ? 0.5 : 1;
}
...
function renderApp(state) {
	...
	const soundBtnIcon = `#icon-sound-on`;
...
function init() {
	...
	// shell size
	options = '';
	['16"'].forEach((opt, i) => options += `<option value="${i}">${opt}</option>`);
